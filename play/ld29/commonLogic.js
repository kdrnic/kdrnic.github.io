...
function ProjectileUpdate()
{
	...
	for(var i = 0; i < numberOfObjects; i++)
	{
		...
		{
			if((obj.IsSolidTo) && (obj.IsSolidTo(this)) && (obj.Hit != this.targetHit))
			{
				this.alive = false;
				return;
			}
...
function DeadBodyUpdate()
{
	if(frame == this.deathFrame) ChangeGameState(4);
	DoPlatformerPhysics(this);
	CameraFocusOn(this);
}

/*
function toRussian(a)
{
	return a.toString().toUpperCase().replace(/B/g, "Б").replace(/T/g, "Г").replace(/A/g, "Д").replace(/X/g, "Ж").replace(/E/g, "З").replace(/N/g, "И").replace(/J/g, "Л").replace(/H/g, "П").replace(/Y/g, "У").replace(/O/g, "Ф").replace(/U/g, "Ц").replace(/W/g, "Ш").replace(/R/g, "Я");
}
*/